...
from dashscope import Generation, Embedding
import dashscope
...
class DashScopeEmbeddings(Embeddings):
    def embed_documents(self, texts):
        ...
        for text in texts:
            try:
                response = Embedding.call(model=Embedding.Models.text_embedding_v2, input=text)
...
# ========== 页面设置 ==========
st.set_page_config(page_title="软工智能助手", layout="wide", initial_sidebar_state="expanded")
st.title("🤖 软件工程智能问答系统")

# ========== 侧边栏 ==========
with st.sidebar:
    st.header("🎛️ 控制面板")
    mode = st.radio("选择问答模式", ["💬 通用问答", "📄 PDF上传问答", "🏆 软工竞赛专区"], index=0)
    st.divider()
...
if mode == "💬 通用问答":
    ...
    if prompt := st.chat_input("请输入你的问题...", key="general_input"):
        ...
        with st.chat_message("assistant", avatar="🤖"):
            with st.spinner("🤔 思考中..."):
                messages = []
                for msg in st.session_state.general_history[-20:]:
...
# ========== PDF上传问答 ==========
    ...
    uploaded_files = st.file_uploader("拖拽或点击上传PDF（支持多个）", type=["pdf"], accept_multiple_files=True, on_change=clear_pdf_state)
    if uploaded_files:
        ...
        if all_documents:
            ...
            qa_chain = ConversationalRetrievalChain.from_llm(llm=DashScopeLLM(), retriever=vectordb.as_retriever(search_kwargs={"k": 4}), memory=memory)
...
# ========== 软工竞赛专区 ==========
    ...
    def build_competition_kb():
        ...
        for pdf_path in glob.glob("knowledge_base/**/*.pdf", recursive=True):
            try:
                ...
                for doc in docs:
                    doc.metadata["category"] = os.path.basename(os.path.dirname(pdf_path))
        ...
        for txt_path in glob.glob("knowledge_base/**/*.txt", recursive=True):
            try:
                ...
                for doc in docs:
                    doc.metadata["category"] = os.path.basename(os.path.dirname(txt_path))
    ...
    st.subheader("🚀 快捷提问")
    quick_questions = ["蓝桥杯常考哪些算法？", "ACM动态规划入门技巧", "单例模式的应用场景", "软件架构设计原则", "如何准备软件工程竞赛？"]
    cols = st.columns(3)
...
请详细回答："""
                response = Generation.call(model="qwen-turbo", messages=[{"role": "user", "content": system_prompt}], result_format="message")
                answer = response.output.choices[0].message.content
                st.markdown(answer)
